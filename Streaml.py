...
def fitness(solution, instances, required_vCPUs, required_memory_GiB):
    ...
    unused_vCPUs = max(0, total_vCPUs - required_vCPUs)
    unused_memory = max(0, total_memory - required_memory_GiB)
    
    if total_vCPUs >= required_vCPUs and total_memory >= required_memory_GiB:
        return 1 / (total_cost + 0.1 * unused_vCPUs + 0.1 * unused_memory)
    return 0
